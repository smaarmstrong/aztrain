...
res = t.get("resources", [])
vmss = [r for r in res if r.get("type") == "Microsoft.Compute/virtualMachineScaleSets"]
...
check("a virtual network resource exists", len(vnets) >= 1)
check("exactly one scale set resource", len(vmss) == 1)

s = vmss[0] if vmss else {}
sku = s.get("sku", {})
check("SKU name is Standard_B1s", sku.get("name") == "Standard_B1s")
check("initial capacity is 2", sku.get("capacity") == 2)
props = s.get("properties", {})
check("upgrade policy mode is Automatic",
      props.get("upgradePolicy", {}).get("mode") == "Automatic")
prof = props.get("virtualMachineProfile", {})
lin = prof.get("osProfile", {}).get("linuxConfiguration", {})
...
osdisk = prof.get("storageProfile", {}).get("osDisk", {})
