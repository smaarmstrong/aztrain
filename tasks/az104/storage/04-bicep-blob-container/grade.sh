#!/usr/bin/env bash
# Compile the learner's Bicep and assert the nested container resources. Read-only.
...
def name_segments(name):
    """Return literal name segments Bicep emitted, whether as a plain 'a/b/c'
    string or a format('{0}/{1}/..', args) expression. Literal (single-quoted)
    args resolve to their value; parameter/other expressions resolve to None."""
...
check("default accessTier is 'Cool'",
      sa.get("properties", {}).get("accessTier") == "Cool")

blob = by_type("Microsoft.Storage/storageAccounts/blobServices")
check("a blobServices resource named 'default'",
      any(name_segments(r.get("name", ""))[-1:] == ["default"] for r in blob))

conts = by_type("Microsoft.Storage/storageAccounts/blobServices/containers")
def seg_ok(r):
    return name_segments(r.get("name", ""))[-1:] == ["exports"]
exports = [r for r in conts if seg_ok(r)]
check("a container named 'exports'", len(exports) >= 1)
check("container 'exports' has publicAccess None",
      any(r.get("properties", {}).get("publicAccess") == "None" for r in exports))
